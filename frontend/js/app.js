...
(function ($) {
    ...
    var $modalBody = $('#table-modal-body', $modal);
    if ($modalBody.length === 0) {
        return;
    }

    $('table > caption', '#content').each(function (index, item) {
        var $item = $(item);
        var $expandButton = $(html);
        $expandButton.data("index", index);        
        $item.prepend($expandButton);

        tableCache[index] = $item.parent("table");
    });

    $('table', '#content').on('click', '.expand-table', function (e) {
        e.preventDefault();
        var $this = $(this);
        ...
        var $table = tableCache[index];

        if ($table.length === 0) {
            return;
        }

        var $body = $table.clone();
        $modalBody.html($body);
